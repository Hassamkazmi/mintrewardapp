...
export default function Routefilters({ data }) {
  ...
  const [selectedValue, setSelectedValue] = useState(data?.singlewaterbody?.WaterBodyType?._id);
  ...
  useEffect(() => {
    if (data && data) {
      form.setFieldsValue({
        RouteAssignment: data?.singlewaterbody?.Service?.map((location, index) => ({
          ...location,
          key: index.toString(),
        })),
      });
...
;
  ...
  const handleupdate = async (values) => {
    const data1 = {
      name: values?.name,
      waterbody_type_id: selectedValue === "" ? data?.WaterBodyType?._id : selectedValue,
      rate: values.rate,
      rate_type_id:  Ratetypeid === "" ?  data?.RateTypeDetail?._id : Ratetypeid,
      labor_cost: values.labor_cost,
      labor_cost_type_id: laborcosttypeid === "" ?  data?.LaborCostDetail?._id : laborcosttypeid,
    ...
    <Fragment>
          ...
          >
            ...
            >
              <div className="row cslocation">
                ...
                <div className="col-sm-12 swicthbtn">
                  <Form.Item
      ...
      <div className="row">
        {postwaterType?.data?.map((item) => (
          <div className="col-sm-2 switchbtn radioBtnAddPoolCustomer" key={item._id}>
            <label>
              <input
                type="radio"
                value={item._id}
              ...
              {item.name}
                ...
                <div className="col-sm-3 routeFilterr">
                  ...
                  >
                    <Select placeholder="Rate Type"  onChange={handleChangeRateType}>
                ...
                <div className="col-sm-3 routeFilterr">
                  ...
                  >
                    <Select placeholder="Labor Cost Type"  onChange={handleChangeLaborCost}>
          ...
          {/* <RouteAssignment data={{data , NewData}} /> */}
        </div>
      </div>
    </Fragment>
  );
