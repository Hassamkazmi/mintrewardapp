...
const PaymentInfoMethod = () => {
    ...
    const {data: getPaymentData , status} = useSelector((state) => state.getPaymentData)
                    ...
                    <div className="row fomik addRoute">
                        ...
                        <form className="formpayement">
                            <div className="card-element">
                                ...
                                {
                                    BrandFunction(getPaymentData?.data?.default_source?.card?.brand)
                                }

                                    
                                </div>
                                <div className="col-sm-12">
                                <h4>**** **** **** {getPaymentData?.data?.default_source?.card?.last4}</h4>
                                <div>
                                <p><b>Expiration: </b><span> {getPaymentData?.data?.default_source?.card?.exp_month}/ {getPaymentData?.data?.default_source?.card?.exp_year}</span></p>
                                </div>
                                </div>
                            </div>
